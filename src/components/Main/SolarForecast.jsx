...
const SolarForecast = () => {
  ...
  const handleFileChange = (event) => {
    ...
    if (selectedFile) {
      ...
      const worker = new Worker(
        new URL("../../config/workerSolar.js", import.meta.url)
      );
      worker.onmessage = (e) => {
  ...
  const uploadJsonToStorage = async (data) => {
    ...
    const filename = `solar_data_${timestamp}.json`;
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen flex flex-col items-center">
      <h1 className="text-2xl font-bold mb-6">
        CSV Reader & File Uploader for Solar Data
      </h1>
      <input
        type="file"
        accept=".csv"
...
export default SolarForecast; // Ensure default export is used
